...
import com.juaracoding.IRspringbootrestapi.model.Cart;
import com.juaracoding.IRspringbootrestapi.repo.CartRepo;
...
public class CartService implements IService<Cart> {
    ...
    public ResponseEntity<Object> save(Cart cart, HttpServletRequest request) {
        ...
        {
            strExceptionArr[1] = "save(Cart cart, HttpServletRequest request) --- LINE 59 \n"+ RequestCapture.allRequest(request);
    ...
    @Override
    public ResponseEntity<Object> update(Long id, Cart cart, HttpServletRequest request) {
        Optional<Cart> opCart;
        Cart cartTrans;
        Boolean isValid = true;

        try{
            opCart =  cartRepo.findById(id);
            ...
            cartTrans = opCart.get();
            //UNTUK METHOD PATCH
//            (cartTrans.getKategoriCart()!=null || !cartTrans.getKategoriCart().equals(""))?cartTrans.setKategoriCart(cart.getKategoriCart()):cartTrans.setKategoriCart(cartTrans.setKategoriCart());
            cartTrans.setIdBarang(cart.getIdBarang());
            cartTrans.setQty(cart.getQty());
        }
        catch (Exception e)
        {
            isValid = false;
            strExceptionArr[1] = "update(Long id, Cart cart, HttpServletRequest request) --- LINE 119 \n"+RequestCapture.allRequest(request);
            LoggingFile.exceptionStringz(strExceptionArr, e, OtherConfiguration.getFlagLoging());
            LogTable.inputLogRequest(logRequestRepo,strExceptionArr,e,OtherConfiguration.getFlagLogTable());
            return new ResponseHandler().generateResponse(
                    "Data tidak Valid",//message
    ...
    public ResponseEntity<Object> delete(Long id, HttpServletRequest request) {
        Optional<Cart> cartTrans =  cartRepo.findById(id);
    ...
    @Override
    public ResponseEntity<Object> saveBatch(List<Cart> lt, HttpServletRequest request) {
        return null;
    }
    ...
    public ResponseEntity<Object> findAll(HttpServletRequest request) {
        ...
        return new ResponseHandler().generateResponse(
                ...
                modelMapper.map(listCart, new TypeToken<List<CartDTO>>() {}.getType()) ,//object
                null,//errorCode diisi null ketika data berhasil disimpan
                request
        );
    }
